...
```{r, fig.cap = "The average number of bacteria in each phyla in all bulk stool samples."}
...
```{r, fig.cap = "Average number of bacteria, by phyla, in all FTA card stool samples."}
library(ggplot2)
library(dplyr)
...
```{r, fig.cap = "Average number of bacteria in FTA and bulk stool samples for all participants."}
...
A Shannon Diversity Index was run with the following code, and was organized by phylum. Phyla Patescibacteria and Cyanobacteria had the lowest values compared to the other phyla. The bulk method shows higher values than the FTA values for all phyla except Patescibacteria and Verrucomicrobiota.
...
```{r, fig.cap = "Shannon Diversity Index by Phylum for both sampling methods."}
#put graphs together
...
The code below was run for a Wilcox test. The p-value was 0.03359.
